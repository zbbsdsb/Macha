...
> （`:protocol` → `dev.macha.layer.protocol`，`:minecraft` → `dev.macha.layer.minecraft`）。
...
└── layers/                             # 【新增】环境接入层：一个 Gradle 构建根，多个模块
    ...
    ├── build.gradle.kts                # 共享配置：toolchain、Kotlin、版本目录、公共测试
    ├── gradle.properties
    ├── gradle/
    ...
    │
    ├── protocol/                       # 【共享】协议 v0：DTO + 编解码 + 版本/能力协商
    ├── runtime/                        # 【共享】LayerRuntime：会话/能力清单/派发/结果关联/节流
    ├── transport/                      # 【共享】HTTP 控制面 + WebSocket 数据面
    │
...
```text
                 ┌──────────────┐
                 │  :protocol   │  纯 JVM：DTO / 编解码 / 版本 / 能力清单模型
                 └──────┬───────┘
            ┌───────────┼───────────────┐
            ▼           ▼               ▼
     ┌────────────┐ ┌──────────┐  ┌───────────────┐
     │ :runtime   │ │:transport│  │ :testclient   │  纯 JVM：垂直切片测试客户端
     └─────┬──────┘ └────┬─────┘  └───────────────┘
           └──────┬──────┘
        ┌─────────┴──────────┐
        ▼                    ▼
...
|---|---|---|---|
| `:protocol` | kotlinx-serialization | 其他一切 | 无 |
| `:runtime` | `:protocol` | `:transport`、`:minecraft`、`:simulator` | 无 |
| `:transport` | `:protocol` | `:runtime`、环境模块 | 无（只有 HTTP/WS 库） |
| `:minecraft` | `:protocol`、`:runtime`、`:transport`、`paper-api`(compileOnly) | — | `org.bukkit`、`io.papermc` |
| `:simulator` | `:protocol`、`:runtime`、`:transport` | `paper-api` | 无 |
| `:testclient` | `:protocol` | `:runtime`、`:transport`、环境模块 | 无 |

**关键性质**：`:runtime` **不依赖** `:transport`——传输实现（HTTP/WS）不得渗进 Layer 逻辑；
...
### 3.1 `layers/protocol/` — 协议 v0 参考实现（纯 JVM）
...
`:runtime` 只认这个接口——"环境接入"因此被压缩成一个可被第二个环境复用的契约。

### 3.3 `layers/transport/` — 传输（HTTP + WS，不认环境）
...
环境特有动作（`minecraft:place_block` 等）将来也只在这里登记，**不进 `:protocol` 的通用词表**。
...
| 建议 | 落点 | 说明 |
|---|---|---|
| `lifecycle/` | `minecraft/plugin/MachaMinecraftPlugin.kt` + `runtime/LayerRuntime.kt` | Bukkit 生命周期与 Layer 生命周期分开，各自归位 |
| `observation/` | `protocol/Payloads.kt`（模型）+ `minecraft/adapter/observe/`（采集） | 模型共享、采集环境特有——最常被弄混的一处 |
| `events/` | `protocol/`（事件 payload）+ `runtime/EventThrottle.kt` + `minecraft/adapter/events/` | 归一化共享，监听器环境特有 |
| `actions/` | `runtime/ActionValidator+Dispatcher`（词表与校验）+ `minecraft/adapter/action/`（执行） | 校验共享、执行环境特有 |
| `entities/` | `protocol/Payloads`（视图）+ `minecraft/adapter/agent/`（绑定） | 不单独建模块：内容不足以成为模块 |
| `protocol/` `transport/` | 原样保留为模块 | 且必须保持零环境依赖 |
...
| 共享内核独立 | `layers/protocol` `runtime` `transport` | `macha-layer-kit`（或留在 Core repo） | 出现第二个独立实现的 Layer（V5 的实证） |
...
1. `layers/` vs 根级 `macha-minecraft/`（本文件推荐 `layers/`，理由见 §1）。
2. 包根 `dev.macha.layer.*` 是否采用（影响全部模块的 import 前缀与 group id）。
3. 是否接受"多 Gradle 模块"（编译期边界）——若否，退化为单模块 + `03` §7 的 import 扫描。
