...
## 9. 试跑期变更记录（2026-09-14 起，冻结前）

> 依据 [`06-next-step-plan.md`](06-next-step-plan.md) §5 **P3：边跑边冻，跑完 8 步再冻结**。
> 规则：任何协议改动**先在下面登记，再改代码**；每条必须回答三问——
> ① 哪条消息/字段变了 ② 真实环境为什么需要它 ③ Simulator Layer 能否表达（判据 J3）。
> 出现"为了让当前 demo 好写"的条目 → 标记 `[DEMO?]` 并停下来复核，不得默认放行。
> 8 步跑完后，本表汇总为 v0 定稿，G1 关闭，本文件去掉 PENDING。
...
action 的 `deadline`/`budget`。这些**等 8 步试跑拿到实测后再决定**是否登记为正式变更；现在只占位。
